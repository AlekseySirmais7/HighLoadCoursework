...
* Сервис статистики
    * Сбор статистики. Предоставляет api для принятия статистики для сервиса объявлений.
    После принятия запроса выполняется запись в ClickHouse. Все параметры записываются в одну таблицу.
    Существуют также таблицы(в ClickHouse, чтобы не грузить другие БД при запросе статистики, нет на схеме БД)
    для информации об объявлении, таргетинге, site.
    Для улучшение скорости работы можно добавить отдельную таблицу - banner_show (объявления, инкремент показов).
...
* Бэк отдача объявлений: 6 80-ти поточных серверов
...
* БД для подбора объявления: 6 32-ти ядерных сервера + 6 шт на реплику

* БД для хранения пользователи с таргетингами: 6 32-ти ядерных сервера + 6 шт на реплику

* БД для сессий рекламодателей/площадки(Redis): 1 16-ти поточный сервер

* Nginx: 9 32-поточных серверов (4 - для статики, 3 для подбора объявления + балансировщик на шарды Tarantool, шарды для таблицы пользователей)
...
* DNS. Укажем несколько IP в А записях. При обращении будет использоваться алгоритм Round Robin.
